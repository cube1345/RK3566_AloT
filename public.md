...
> 最后更新: 2026-05-04 | 版本: v5.3 | 传感器: SGP30+DHT11+BH1750+HC-SR501+K210 | 测试: 152 passed
...
### 决策架构 (v5.2 升级)
- **CoT思考链** (v5.3统一): LLM输出统一为 观察→分析→决策, AIBrain+CommandHandler双引擎一致格式, 推理可追溯
...
7. ⑦ Web Dashboard 总控 (SSE流式CoT可视化, 左右分栏对话UI, 17 API)
...
- [ ] Pi 5 部署 (LLM segfault 修复中)
  ...
  - [x] 入口函数规范 (main.run())
  - [x] LLM 性能优化 (n_threads 2→4, n_batch 512, CoT格式统一)
- [x] 前端重构 (SSE流式CoT可视化, 左右分栏对话UI, 阶段动画)
